...
<AccordionGroup>
  ...
  <Accordion title="Retrieve logo for FTSE All-World UCITS ETF using its ticker">
    Let's say you want to retrieve the logo for the FTSE All-World UCITS ETF using its ticker `VWRL` traded on the London Stock Exchange. You can do this by using the following URL:

    ```html
    <img src="https://img.logokit.com/ticker/VWRL:LN?token=<publishable-api-token>" alt="FTSE All-World UCITS ETF logo (identification only, no affiliation)" />
    ```
